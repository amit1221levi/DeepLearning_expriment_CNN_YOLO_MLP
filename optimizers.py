...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        # ====== YOUR CODE: ======
        self.v_t = [0 for p, dp in params if dp is not None]
        # ========================
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            dp += self.reg * p
            v_t1 = self.momentum * self.v_t.pop(0) - self.learn_rate * dp
            p += v_t1
            self.v_t.append(v_t1)
            # ========================
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        # TODO: Add your own initializations as needed.
        # ====== YOUR CODE: ======
        self.r_t = [0 for p, dp in params if dp is not None]
        # ========================
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            dp += self.reg * p
            r_t1 = self.decay * self.r_t.pop(0) + (1-self.decay) * torch.pow(dp, 2)
            p -= ((self.learn_rate/ torch.sqrt(r_t1+self.eps)) * dp)
            self.r_t.append(r_t1)
            # ========================
