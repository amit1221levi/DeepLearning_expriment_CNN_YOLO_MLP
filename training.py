    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            epochTrainResult = self.train_epoch(dl_train, verbose=verbose, **kw)
            epochTestResult = self.test_epoch(dl_test, verbose=verbose, **kw)
            
            train_acc += [epochTrainResult.accuracy]
            train_loss += [sum(epochTrainResult.losses)/len(epochTrainResult.losses)]
           
            test_acc += [epochTestResult.accuracy]
            test_loss += [sum(epochTestResult.losses)/len(epochTestResult.losses)]
            # ========================
...
class ClassifierTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        classifications = self.model.classify_scores(y_pred)
        num_correct = (classifications == y).sum().item()
        # ========================
...
class LayerTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        num_correct = (torch.argmax(y_pred, 1) == y).sum().item()
        # ========================
